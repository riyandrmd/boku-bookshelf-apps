...
const UpdateBook = () => {
    const { id } = useParams();
    const { thisBooks } = useContext(dataContext)
    ...
    useEffect(() => {
        if (Object.values(newBook).every((value) => !value)) {
            setNewBook(thisBooks);
        }
    }, [newBook, thisBooks]);
    ...
    const handleSave = async () => {
        try {
            await Api('books/' + id, "PUT", newBook);
            alert("Book Updated!");
        } catch (error) {
